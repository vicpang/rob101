...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
		...
		mu(2*landmarkId+2:2*landmarkId+3,1)=mu(1:2,1) +...
            [z(i).range*cos(z(i).bearing+mu(3,1));z(i).range*sin(z(i).bearing+mu(3,1))];
    ...
    delta=mu(2*landmarkId+2:2*landmarkId+3,1)-mu(1:2,1);
    q=delta'*delta;
    expectedZ(2*i-1:2*i,1)=[sqrt(q);normalize_angle(atan2(delta(2,1),delta(1,1))-mu(3,1))];
	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	Fx=zeros(5,size(mu,1));
    Fx(1:3,1:3)=eye(3);
    Fx(4,2*landmarkId+2)=1;
    Fx(5,2*landmarkId+3)=1;
    Hi=1/q*[-sqrt(q)*delta(1,1) -sqrt(q)*delta(2,1) 0 sqrt(q)* delta(1,1) sqrt(q)*delta(2,1);...
        delta(2,1) -delta(1,1) -q -delta(2,1) delta(1,1)];
    Hi=Hi*Fx;
