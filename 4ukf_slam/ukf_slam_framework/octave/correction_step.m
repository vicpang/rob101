...
for i = 1:m
	...
	if isempty(find(map == z(i).id,1))
	    % Add new landmark to the map
            [mu, sigma, map] = add_landmark_to_map(mu, sigma, z(i), map, Q);
        % first time viewing a landmark, no correction needed
	...
	% TODO: Compute z_points (2x2n+1), which consists of predicted measurements from all sigma points
        ...
        for j=1:2*n+1
		z_points(:,j) = [sqrt((landmarkXs(j)-sigma_points(1,j))^2 + ...
            (landmarkYs(j)-sigma_points(2,j))^2); ...
            normalize_angle(atan2(landmarkYs(j)-sigma_points(2,j),...
            landmarkXs(j)-sigma_points(1,j)) - sigma_points(3,j))]	
        end
